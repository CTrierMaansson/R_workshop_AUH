...
Use the commands above or go to File -> New file etc. 
You can go to the Pathfinder(MS)/Finder(IOS) to create the directories
...
We now try to create some functions using the built-in data set `iris`. 
...
*Hint: use len <- mean(data$Sepal.Length) and list('Sepal.Length_mean' = len)*
...
`variable` is a column name existing in `iris`. Use `variable` to extract values
from `iris` and calculate the average. 
The function should still return a named list with the average value

*Hint: use values <- data[[variable]], label <- paste0(variable,"_mean")*,
*and names(list()) <- label*
...
*Hint: use *`if(!is.numeric()){stop()}` and `if(!is.character()){stop()}`
...
*Hint: use vars <- colnames(data) and if(!(variable %in% vars)){stop()}*
...
```{r, highlight=T,message=F,eval=FALSE,echo = T,class.source="fold-show"}
# If you DONT have the packages installed run:
...
common entries, where the column name typically is identical.
...
1. Create a function `tidy_func_1()` which takes the iris data set as default `data` argument 
...
And run the following commands to create `mtcars_mod` from the built-in `mtcars`:
...
argument. Remove the variables `vs`,`disp`,`drat`,`wt` and `am`. 
Then filter cars also present in
...
simple. By saving the data you do not have to re-analyze the data and you can 
move quickly to stats, plots, etc. 
...
df <- data.frame(functions = c("saveRDS(<object>, <path>)",
                 ...
                 description = c("Save an object in the designated path, finish the path with <file_name>.rds",
                                 ...
                                 "Save a table in the designated path, finish the path with <file_name>.txt",
...
*HINT: I always suggest to write the FULL PATH when saving and reloading.*
*Use `getwd()` to get the full path to your current directory*
...
`aes` stands for aesthetics which encompass what variables from the data set can 
affect how the plot looks like in the end. 
...
e.g. lines and points. Like this:
...
Here a few examples of how `geoms` can be used:
...
ggplot(mtcars_mod_sele, aes(x=mpg,y=hp))+
...
```{r, highlight=T,message=F,eval=F,echo = T,class.source="fold-show"}
# If you DONT have the package installed run:
...
```{r, echo=FALSE, fig.cap="Fig 7", out.width='30%'}
